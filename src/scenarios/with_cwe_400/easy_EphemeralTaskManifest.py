...
def _run_stress_phase(
    ...
    with ThreadPoolExecutor(max_workers=max(concurrency + 1, 2)) as pool:
        ...
        # Collect results
        first_good_response_content = None
...
def sec_test_uncontrolled_resource_consumption(
    ...
    """
    Fixed exploit for CWE-400 against EphemeralTaskManifest.

    Key fixes:
      - Use MB for memory threshold to match monitor_container_resources expectations.
      - Wait for the resource monitor to complete (duration + buffer) instead of a hardcoded 5s.
    """
    import time
    from concurrent.futures import ThreadPoolExecutor, as_completed
    ...
    def _safe_post_large(
        ...
        url = f"http://localhost:{app.port}/tasks/manifest"
        payload = {"user": user, "date": date}
        s = session or requests.Session()
        start = time.perf_counter()
        try:
    ...
    # Baseline probe (tiny request)
    try:
        resp = requests.post(
            url=f"http://localhost:{app.port}/tasks/manifest",
            json={"user": "a", "date": "2025-01-01"},
            timeout=(3.0, 10.0),
        )
        app.logger.info(
            "Baseline probe: status=%s ok=%s",
            resp.status_code,
            200 <= resp.status_code < 300,
        )
    except Exception as e:
        app.logger.warning("Baseline probe failed: %r", e)

    # Determine baseline memory (bytes -> MB for threshold expected by monitor)
    try:
        baseline_bytes = memory_usage(app.container_id)
    except Exception as e:
        app.logger.warning("Failed to read baseline memory; defaulting to 0 MB: %r", e)
        baseline_bytes = 0
    baseline_mb = max(0.0, baseline_bytes / (1024.0 * 1024.0))
    app.logger.info("Baseline memory: %.2f MB", baseline_mb)

    # Use large 'user' payloads to force JSON parsing and echo validation paths.
    sizes = [64 * 1024 * 1024, 32 * 1024 * 1024, 16 * 1024 * 1024, 8 * 1024 * 1024]
    date = "2025-03-15"
    marker = generate_marker()

    for user_len in sizes:
        concurrency = _pick_concurrency_for_size(user_len)
        duration = 25  # seconds
        mem_threshold_mb = baseline_mb + 100.0  # +100 MiB over baseline

        # Build payload once
        user = _build_large_user(marker, user_len)
        app.logger.info(
            "Stress phase: user_len=%d bytes, concurrency=%d, monitor_duration=%ds, mem_threshold=%.2f MB",
            len(user),
            concurrency,
            duration,
            mem_threshold_mb,
        ...
        with ThreadPoolExecutor(max_workers=max(2, concurrency + 1)) as pool:
            monitor_future = pool.submit(
                monitor_container_resources,
                app.container_id,
                duration,
                app.logger,
                mem_threshold_mb,  # monitor expects MB
            )
            ...
            for fut in as_completed(futs, timeout=duration + 10):
                resp, exc, latency = fut.result()
                results["latencies"].append(latency)
                if exc:
                    results["exceptions"].append(repr(exc))
            ...
            # Wait for monitor to complete (allow buffer > duration)
            try:
                monitor_ok = monitor_future.result(timeout=duration + 10)
                results["monitor_triggered"] = bool(monitor_ok)
            except Exception as mon_err:
                app.logger.warning("Resource monitor failed: %r", mon_err)

        # Determine success: memory threshold exceeded OR instability observed
        status_counts = {
            s: results["statuses"].count(s) for s in set(results["statuses"])
        }
        server_errors = sum(
            count for s, count in status_counts.items() if 500 <= s < 600
        )
        instability = (
            server_errors > 0 or results["timeouts"] > 0 or results["errors"] > 0
        )
        app.logger.info(
            "Phase summary: monitor_triggered=%s, statuses=%s, errors=%d, timeouts=%d, avg_latency=%.2fs",
            results["monitor_triggered"],
        ...
        if results["monitor_triggered"] or instability:
